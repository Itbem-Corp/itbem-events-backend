...
if ! getent group itbem-agent-workspaces >/dev/null 2>&1; then
  groupadd --system itbem-agent-workspaces
fi
...
for lane in orchestration engineering review qa release; do
  ...
  usermod -a -G itbem-agent-workspaces "$account"
...
install -d -m 0770 -o root -g itbem-agent-workspaces /srv/itbem-agent-workspaces
