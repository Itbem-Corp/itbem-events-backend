...
func buildTaskMessages(operation string, input TaskInput, lookup func(string) string) ([]Message, error) {
	...
	if operation == "code.review" {
		messages[0].Content += ` STRICT JSON TYPE CONTRACT: review_scope, test_plan, and coverage_gaps are arrays of plain JSON strings only, never arrays of objects. review_scope, test_plan, and coverage_gaps each contain at most 12 items; findings contains at most 24 objects. A minimal valid shape is {"summary":"Checked the frozen change.","verdict":"approve","review_scope":["authentication flow","regression tests"],"findings":[],"test_plan":["Run go test ./..."],"coverage_gaps":[]}. Do not add properties to string-array items. Before responding, verify every opening bracket is closed and every array element has the required JSON type.`
