...
func (w *Worker) Process(ctx context.Context, message TaskMessage) error {
	...
	if message.Payload.Operation == "delivery.implementation" {
		if required, declared := approvedChangedRepositoryReferences(input.Delivery); declared && len(required) == 0 {
			return w.fail(ctx, message.Payload.TaskID, runID, fmt.Errorf("approved plan declares no repository changes; use delivery.assessment instead of implementation"))
		}
	}
	...
	if message.Payload.Operation == "delivery.assessment" {
		...
		if verdict, _ := structuredResult["verdict"].(string); verdict != "assessed" {
			return w.failWithProviderResult(ctx, message.Payload.TaskID, runID, requestRef, message.Payload.Operation, completion, fmt.Errorf("read-only assessment is blocked; inspect the recorded evidence before retrying"))
		}
		if required, declared := approvedChangedRepositoryReferences(input.Delivery); !declared || len(required) != 0 {
			return w.failWithProviderResult(ctx, message.Payload.TaskID, runID, requestRef, message.Payload.Operation, completion, fmt.Errorf("read-only assessment requires an approved plan with zero changed repositories"))
		}
		execution = readOnlyAssessmentHandoff()
	}
